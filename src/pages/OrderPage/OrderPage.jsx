...
const OrderPage = () => {
  ...
  const fetchUserId = async (email) => {
    try {
      ...
      if (!text) throw new Error("Empty response from user ID API");
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (!text) throw new Error("Empty response from orders API");
        
        const orders = JSON.parse(text);
        if (!Array.isArray(orders)) throw new Error("Received data is not an array");
        
        const completeOrders = await Promise.all(
          orders.map(async (order) => {
            try {
              const itemsResponse = await fetch(`http://${abhishekIp}:${orderPort}/orders/items/${order.oid}`);
              if (!itemsResponse.ok) throw new Error(`Failed to fetch order items for Order ID: ${order.oid}`);
              
              const itemsText = await itemsResponse.text();
              if (!itemsText) return [];
              
              const orderItems = JSON.parse(itemsText);
              console.log("orderItems:", orderItems);
              ...
              return Promise.all(
                orderItems.map(async (item) => {
                  try {
                    const productResponse = await fetch(`http://${abhishekIp}:8095/products/product/${item.productId}`);
                    if (!productResponse.ok) throw new Error(`Failed to fetch product details for Product ID: ${item.productId}`);
