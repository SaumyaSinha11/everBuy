...
const UserProfileCard = () => {
    ...
    const validateName = (name) => {
        if (/^[\x41-\x7A\u00C0-\uD7FB\ \']+([\-])*[.]{0,1}$/.test(name) && name.length < 50 && name.length != 0) return true;
        return false;
    };
    ...
    const handleEdit = async (e) => {
        ...
        if (!validateCode(user.phoneCountryCode)) errors.phoneCountryCode = "Invalid country code"
        ...
        if (Object.keys(errors).length === 0) {
            ...
            try {
                const response = await fetch(`http://${saumyaIp}:${userPort}/users/${user.email}`, {
                    method: 'PUT',
                    headers: {
...
>
  ...
  <Link to="/user" style={{ textDecoration: "none", fontWeight: "bold", marginTop: "10px" }}>
    Log In
  </Link>
</div>:
                <Card sx={{ maxWidth: 400, mx: "auto", mt: 5, p: 2, boxShadow: 3, borderRadius: 3 }}>
                    <CardContent>
                        <TextField
                            fullWidth
                            label="Name"
                        ...
                        <Box sx={{ display: 'flex', gap: '1', mt: 4 }}>
                            <TextField
                                ...
                                onChange={change ? undefined : handleEditChange("phoneCountryCode")}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'end', marginTop: '2rem', gap: 5 }}>
                            {change ? <>
                                ...
                                : <>
                                    <Button onClick={() => { setChange(true) }}>Cancel</Button>
                                    {isDelete ?
